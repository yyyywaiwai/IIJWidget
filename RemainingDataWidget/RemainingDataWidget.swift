...
struct RemainingDataProvider: AppIntentTimelineProvider {
  ...
  func snapshot(for configuration: RemainingDataConfigurationIntent, in context: Context) async
    -> RemainingDataEntry
  {
    if context.isPreview {
      return RemainingDataEntry(
        date: Date(), snapshot: .placeholder, accentColors: accentStore.load())
    }
  ...
  func timeline(for configuration: RemainingDataConfigurationIntent, in context: Context) async
    -> Timeline<RemainingDataEntry>
  {
    if context.isPreview {
      let previewEntry = RemainingDataEntry(
        date: Date(), snapshot: .placeholder, accentColors: accentStore.load())
  ...
  private func resolveAccentColors(from configuration: RemainingDataConfigurationIntent)
    -> AccentColorSettings
  {
...
struct RemainingDataWidgetEntryView: View {
    ...
    Group {
      if showSuccess {
        ...
        Gauge(value: service.remainingGB, in: 0...service.totalCapacityGB) {
          ...
          gbValueTextCompact(service.remainingGB, baseSize: valueFontSize, weight: .semibold)
        }
  ...
  private var rectangularView: some View {
    VStack(alignment: .leading, spacing: 4) {
      if let service = entry.snapshot?.primaryService {
        ...
        .frame(height: 8)
        Text(
          "残量 \(formattedGBWithUnit(service.remainingGB)) / \(formattedGBWithUnit(service.totalCapacityGB))"
        )
        .font(.footnote)
      } else {
        Text("IIJmioデータ未取得")
          ...
          HStack(spacing: 8) {
            Image(systemName: "simcard.fill")
              ...
              .foregroundStyle(
                LinearGradient(
                  colors: ringColors(
                    for: entry.snapshot?.primaryService.map { remainingRatio(for: $0) } ?? 0.5),
            ...
            Text(
              showSuccess
                ? "更新完了"
                : (isRefreshing
                  ? "更新中..." : (entry.snapshot?.primaryService?.serviceName ?? "IIJmio"))
            )
  ...
  private func circularMeter(
    for service: WidgetServiceSnapshot, size: CGFloat, lineWidth: CGFloat = 10
  ) -> some View {
  ...
  /// ロック画面の小ウィジェット用：3桁表示（小数点以下1桁）で読みやすく
  private func gbValueTextCompact(
    _ value: Double,
    baseSize: CGFloat,
    weight: Font.Weight
  ) -> Text {
...
struct RemainingDataWidget: Widget {
  ...
  var body: some WidgetConfiguration {
    AppIntentConfiguration(
      kind: kind, intent: RemainingDataConfigurationIntent.self, provider: RemainingDataProvider()
    ) { entry in
