...
extension UsageChartPoint {
  static func == (lhs: UsageChartPoint, rhs: UsageChartPoint) -> Bool {
    lhs.displayLabel == rhs.displayLabel && lhs.rawKey == rhs.rawKey && lhs.value == rhs.value
      && lhs.sortKey == rhs.sortKey && lhs.date == rhs.date
  }
}
...
extension String {
  fileprivate var numericIdentifier: Int {
    Int(filter(\.isNumber)) ?? 0
  }
}

extension Date {
  fileprivate var chartSortKey: Int {
